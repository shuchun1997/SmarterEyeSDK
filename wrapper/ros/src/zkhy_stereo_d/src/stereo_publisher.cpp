...
void StereoPublisher::publishFrameCallback(int frameId, int64_t timestamp, const cv::Mat &img_mat)
{
    ...
    if (frameId == FrameId::CalibLeftCamera || frameId == FrameId::CalibRightCamera) {
        ...
        // img_msg->header.stamp = ros::Time(timestamp/1000);
        header.stamp = ros::Time().now();   
        // img_msg->header.stamp = ros::Time::now();
        std::cout<< "timestamp = "<< header.stamp << std::endl;
       
        if (frameId == FrameId::CalibLeftCamera) {
            if (is_color) {
              //img_msg->header.frame_id = "zkhy_left_color_frame";
                header.frame_id = "zkhy_left_color_frame";
                img_msg = cv_bridge::CvImage(header, encoding, img_mat).toImageMsg();
                left_color_pub_.publish(img_msg);

             // color to gray
	        cv::Mat left_gray;
	        cv::cvtColor(img_mat, left_gray, CV_BGR2GRAY);
	      //sensor_msgs::ImagePtr img_msg = cv_bridge::CvImage(header, "mono8", left_gray).toImageMsg();
              //img_msg->header.frame_id = "zkhy_left_gray_frame";
                header.frame_id = "zkhy_left_gray_frame";
              //left_gray_msg->header.stamp = img_msg->header.stamp;
                sensor_msgs::ImagePtr img_msg = cv_bridge::CvImage(header, "mono8", left_gray).toImageMsg();
        ...
        if (frameId == FrameId::CalibRightCamera) {
           //img_msg = cv_bridge::CvImage(header encoding, img_mat).toImageMsg();   //hahaha
            if (is_color) {
                //img_msg->header.frame_id = "zkhy_right_color_frame";
                header.frame_id = "zkhy_right_color_frame";
                right_color_pub_.publish(img_msg);
            } else {
               // img_msg->header.frame_id = "zkhy_right_gray_frame";
                header.frame_id = "zkhy_right_gray_frame";
                img_msg = cv_bridge::CvImage(header, encoding, img_mat).toImageMsg();
